...
class ReturnComprehensiveSeeder extends Seeder
{
    ...
    private function seedSewaReturns($buyers, $sellers)
    {
        ...
        $seller = $sellers->where('email', 'slamet.outdoor@gmail.com')->first()
            ?? $sellers->first();
        ...
        if ($product) {
            ...
            ReturnEscrow::create([
                ...
                'type' => 'sewa',
                'status' => 'dispute',
                'escrow_total' => $rentPrice + $deposit,
        ...
        if ($buyer2 && $seller2 && $seller2->store) {
            ...
            $product2 = Product::where('store_id', $seller2->store->id)
                ->where('is_rental', true)
                ->first();
