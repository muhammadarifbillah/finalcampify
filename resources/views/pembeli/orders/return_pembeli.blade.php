...
<div class="pt-28 pb-20 bg-slate-50 min-h-screen">
    <div class="max-w-2xl mx-auto px-4">
        ...
        <!-- Progress Stepper (Khusus Beli / Purchase) -->
        @if($detail->type === 'buy')
        <div class="bg-white rounded-[24px] shadow-sm border border-slate-100 p-6 mb-6">
            <div class="flex items-center justify-between text-[11px] font-bold uppercase tracking-wider text-slate-400">
                <span>Status Retur</span>
                <span class="text-rose-600 font-extrabold">Beli</span>
            </div>
            ...
            <div class="mt-4 flex items-center justify-between relative">
                ...
                @php
                    ...
                    $step = 1;
                    if ($status === 'pending') $step = 2;
                    if (in_array($status, ['approved', 'refund_pending', 'replacement_shipping'])) $step = 3;
                ...
                <!-- Step 1: Pengajuan -->
                <div class="z-10 flex flex-col items-center">
                    <div class="w-8 h-8 rounded-full flex items-center justify-center font-bold text-xs {{ $step >= 1 ? 'bg-rose-600 text-white shadow-md shadow-rose-200' : 'bg-slate-100 text-slate-400' }}">1</div>
                    <span class="text-[9px] font-bold text-slate-500 mt-2">Pengajuan</span>
                </div>
                
                <!-- Step 2: Tinjauan Seller -->
                <div class="z-10 flex flex-col items-center">
                    <div class="w-8 h-8 rounded-full flex items-center justify-center font-bold text-xs {{ $step >= 2 ? 'bg-rose-600 text-white shadow-md shadow-rose-200' : 'bg-slate-100 text-slate-400' }}">2</div>
                    <span class="text-[9px] font-bold text-slate-500 mt-2">Tinjauan Seller</span>
                </div>

                <!-- Step 3: Resolusi -->
                <div class="z-10 flex flex-col items-center">
                    <div class="w-8 h-8 rounded-full flex items-center justify-center font-bold text-xs {{ $step >= 3 ? 'bg-rose-600 text-white shadow-md shadow-rose-200' : 'bg-slate-100 text-slate-400' }}">3</div>
                    <span class="text-[9px] font-bold text-slate-500 mt-2">Resolusi</span>
                </div>

                <!-- Step 4: Selesai -->
                <div class="z-10 flex flex-col items-center">
                    <div class="w-8 h-8 rounded-full flex items-center justify-center font-bold text-xs {{ $step >= 4 ? 'bg-rose-600 text-white shadow-md shadow-rose-200' : 'bg-slate-100 text-slate-400' }}">4</div>
                    <span class="text-[9px] font-bold text-slate-500 mt-2">Selesai</span>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-[32px] shadow-xl p-8 border border-slate-100">
            ...
            @else
                <!-- ================== ALUR JUAL BELI (PEMBELIAN - BERDASARKAN BPMN) ================== -->
                @if(!$return)
                    <!-- STAGE 1: AJUKAN KOMPLAIN -->
                    <form action="{{ route('orders.return.store', $detail->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                        @csrf
                        <div>
                            <label class="block text-sm font-bold text-slate-700 mb-2">Alamat Toko (Tujuan Pengembalian)</label>
                            <div class="p-4 bg-rose-50 text-rose-800 rounded-2xl text-sm border border-rose-100">
                                <p class="font-bold text-base">{{ $detail->product->store->nama_toko ?? 'Toko Campify' }}</p>
                                <p class="mt-1 text-slate-600">{{ $detail->product->store->alamat ?? 'Alamat Toko tidak terintegrasi' }}</p>
                                <p class="mt-2 text-[11px] opacity-75 font-semibold">Toko ini yang bertanggung jawab untuk menerima pengembalian barang Anda kelak.</p>
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 bg-slate-50 p-6 rounded-[24px] border border-slate-100">
                            ...
                            <div class="space-y-1">
                                <label class="block text-[10px] font-black text-rose-500 uppercase tracking-widest">Estimasi Refund</label>
                                <p class="text-sm font-bold text-rose-600">100% (Rp {{ number_format($detail->harga * $detail->qty) }})</p>
                            </div>
                        </div>

                        <div>
                            <label class="block text-sm font-bold text-slate-700 mb-2">Alasan Komplain / Masalah Barang</label>
                            <textarea name="alasan_return" rows="3" class="w-full rounded-2xl border-slate-200 p-4 focus:ring-rose-500 focus:border-rose-500 text-sm" placeholder="Jelaskan detail masalah barang Anda (misal: barang rusak, tidak sesuai deskripsi, dll)" required></textarea>
                        </div>

                        <div>
                            <label class="block text-sm font-bold text-slate-700 mb-2">Foto Bukti Masalah Barang</label>
                            <div class="border-2 border-dashed border-slate-200 rounded-2xl p-6 text-center hover:border-rose-400 transition cursor-pointer relative bg-slate-50">
                                <input type="file" name="foto_kondisi" accept="image/*" class="w-full text-sm text-slate-600" required />
                                <p class="mt-2 text-[10px] text-slate-500">Silakan unggah foto fisik barang yang bermasalah.</p>
                            </div>
                        </div>

                        <!-- Input Rekening Bank Pembeli (Jika resolusinya Refund) -->
                        <div class="bg-slate-50 p-6 rounded-[24px] border border-slate-100 space-y-4">
                            <h4 class="text-sm font-bold text-slate-800">Rekening Pengembalian Dana (Refund)</h4>
                            <p class="text-xs text-slate-500">Isi rekening bank Anda untuk berjaga-jaga jika Penjual memilih resolusi Pengembalian Uang (Refund Dana).</p>
                            
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    <label class="block text-xs font-bold text-slate-600 mb-1">Nama Bank</label>
                                    <input type="text" name="buyer_refund_bank_name" class="w-full text-xs rounded-xl border-slate-200 p-3 focus:ring-rose-500 focus:border-rose-500" placeholder="Contoh: BCA, Mandiri, BRI" required>
                                </div>
                                <div>
                                    <label class="block text-xs font-bold text-slate-600 mb-1">Nomor Rekening</label>
                                    <input type="text" name="buyer_refund_bank_account" class="w-full text-xs rounded-xl border-slate-200 p-3 focus:ring-rose-500 focus:border-rose-500" placeholder="Nomor rekening tanpa spasi" required>
                                </div>
                                <div class="col-span-1 md:col-span-2">
                                    <label class="block text-xs font-bold text-slate-600 mb-1">Nama Pemilik Rekening</label>
                                    <input type="text" name="buyer_refund_bank_name_owner" class="w-full text-xs rounded-xl border-slate-200 p-3 focus:ring-rose-500 focus:border-rose-500" placeholder="Nama lengkap sesuai buku tabungan" required>
                                </div>
                            </div>
                        </div>

                        <button type="submit" class="w-full bg-rose-600 hover:bg-rose-700 text-white py-4 rounded-2xl font-bold shadow-lg shadow-rose-200 transition-all transform hover:-translate-y-1">
                            Ajukan Komplain Retur
                        </button>
                    </form>

                @elseif($return && $return->status === 'pending')
                    <!-- STAGE 2: MENINJAU KOMPLAIN -->
                    <div class="text-center py-10 space-y-4">
                        <div class="w-20 h-20 bg-amber-50 text-amber-500 rounded-full flex items-center justify-center mx-auto mb-4 animate-bounce">
                            <i data-lucide="clock" class="w-10 h-10"></i>
                        </div>
                        <h3 class="text-xl font-bold text-slate-800">Meninjau Komplain</h3>
                        <p class="text-slate-500 max-w-md mx-auto text-sm">
                            Permintaan komplain Anda telah berhasil diajukan. Penjual sedang memeriksa rincian masalah dan foto bukti yang Anda kirimkan untuk validasi kelayakan retur.
                        </p>
                        
                        <div class="mt-8 bg-slate-50 p-6 rounded-2xl text-left border border-slate-100 space-y-4">
                            <h4 class="text-xs font-bold uppercase tracking-wider text-slate-400">Rangkuman Komplain Anda</h4>
                            <div class="grid grid-cols-3 gap-2 text-xs">
                                <span class="text-slate-400">Masalah:</span>
                                <span class="col-span-2 font-bold text-slate-800">{{ $return->renter_notes }}</span>
                                <span class="text-slate-400">Rekening Refund:</span>
                                <span class="col-span-2 font-bold text-slate-800">{{ $return->buyer_refund_bank_name }} - {{ $return->buyer_refund_bank_account }} a/n {{ $return->buyer_refund_bank_name_owner }}</span>
                            </div>

                            @if($return->proof_returned_image)
                            <div class="mt-4 pt-4 border-t border-slate-200">
                                <p class="text-[10px] font-bold text-slate-400 uppercase mb-2">Bukti Fisik Barang</p>
                                <a href="{{ asset($return->proof_returned_image) }}" target="_blank" class="block rounded-xl overflow-hidden border border-slate-200">
                                    <img src="{{ asset($return->proof_returned_image) }}" class="w-full max-h-48 object-cover">
                                </a>
                            </div>
                            @endif
                        </div>
                ...
                @elseif($return && $return->status === 'rejected')
                    ...
                    <div class="text-center py-10 space-y-4">
                        ...
                        <h3 class="text-xl font-bold text-slate-800">Komplain Tidak Valid / Ditolak</h3>
                        <p class="text-slate-500 max-w-md mx-auto text-sm">
                            Maaf, komplain yang Anda ajukan dinilai tidak valid atau ditolak oleh Penjual.
                        </p>
                        @if($return->owner_notes)
                        <div class="bg-rose-50 p-4 rounded-xl text-left border border-rose-100 text-xs text-rose-800 max-w-md mx-auto">
                            <strong>Alasan Penolakan:</strong> {{ $return->owner_notes }}
                        </div>
                        @endif
                    </div>

                @elseif($return && $return->status === 'refund_pending')
                    <!-- STAGE 3A: PROSES REFUND DANA -->
                    <div class="text-center py-10 space-y-4">
                        <div class="w-20 h-20 bg-emerald-50 text-emerald-500 rounded-full flex items-center justify-center mx-auto mb-4 animate-pulse">
                            <i data-lucide="banknote" class="w-10 h-10"></i>
                        </div>
                        <h3 class="text-xl font-bold text-slate-800">Proses Refund Dana</h3>
                        <p class="text-slate-500 max-w-md mx-auto text-sm">
                            Penjual telah **menyetujui komplain Anda** dengan resolusi **Pengembalian Dana (100% Refund)**. 
                        </p>
                        <p class="text-xs text-slate-400">
                            Sedang memproses transfer dana sebesar <strong>Rp {{ number_format($return->escrow_total) }}</strong> ke rekening Anda:
                        </p>

                        <div class="bg-slate-50 p-6 rounded-2xl text-left border border-slate-100 space-y-3 max-w-md mx-auto">
                            <h4 class="text-xs font-bold uppercase tracking-wider text-slate-400">Rekening Tujuan</h4>
                            <div class="text-xs space-y-1">
                                <p><span class="text-slate-400">Bank:</span> <span class="font-bold text-slate-800">{{ $return->buyer_refund_bank_name }}</span></p>
                                <p><span class="text-slate-400">Nomor Rekening:</span> <span class="font-bold text-slate-800">{{ $return->buyer_refund_bank_account }}</span></p>
                                <p><span class="text-slate-400">Pemilik Rekening:</span> <span class="font-bold text-slate-800">{{ $return->buyer_refund_bank_name_owner }}</span></p>
                                <p class="pt-2 border-t mt-2 text-rose-600 font-extrabold text-sm flex justify-between">
                                    <span>Total Refund:</span>
                                    <span>Rp {{ number_format($return->escrow_total) }}</span>
                                </p>
                            </div>
                        </div>
                        <p class="text-[10px] text-slate-400 italic">Harap tunggu hingga Seller / Admin memverifikasi dan mengirimkan dana refund.</p>
                    </div>

                @elseif($return && $return->status === 'approved')
                    <!-- STAGE 3B: PROSES REPLACEMENT (MENUNGGU KIRIM) -->
                    <div class="text-center py-10 space-y-4">
                        ...
                        <h3 class="text-xl font-bold text-slate-800">Menunggu Pengiriman Produk Pengganti</h3>
                        <p class="text-slate-500 max-w-md mx-auto text-sm">
                            Penjual telah **menyetujui komplain Anda** dengan resolusi **Kirim Produk Pengganti**. 
                        </p>
                        <p class="text-xs text-slate-400">
                            Seller sedang menyiapkan produk pengganti baru yang sesuai untuk dikirimkan ke alamat Anda.
                        </p>
                        <p class="text-[10px] text-slate-400 italic">Nomor resi pengiriman pengganti akan langsung muncul di sini setelah diinput oleh Seller.</p>
                    </div>

                @elseif($return && $return->status === 'replacement_shipping')
                    <!-- STAGE 3C: PROSES REPLACEMENT (SEDANG DIKIRIM & TOMBOL KONFIRMASI) -->
                    <div class="text-center py-10 space-y-4">
                        <div class="w-20 h-20 bg-rose-100 text-rose-600 rounded-full flex items-center justify-center mx-auto mb-4">
                            <i data-lucide="truck" class="w-10 h-10 animate-bounce"></i>
                        </div>
                        <h3 class="text-xl font-bold text-rose-600">Produk Pengganti Sedang Dikirim!</h3>
                        <p class="text-slate-500 max-w-md mx-auto text-sm">
                            Penjual telah mengirimkan produk pengganti baru untuk Anda.
                        </p>

                        <div class="bg-rose-50 p-6 rounded-2xl text-left border border-rose-100 max-w-md mx-auto space-y-2">
                            <p class="text-xs text-rose-800"><span class="font-bold">Kurir Pengganti / Resi:</span></p>
                            <p class="text-lg font-black text-rose-600 select-all">{{ $return->resi_pengganti }}</p>
                            <p class="text-[10px] text-slate-400 leading-relaxed pt-1">
                                Silakan lacak pengiriman barang pengganti di atas. Jika paket sudah tiba di tangan Anda dan kondisinya sudah sesuai, silakan konfirmasi retur.
                            </p>
                        </div>

                        <form action="{{ route('orders.return.confirm-replacement', $return->id) }}" method="POST" class="pt-6 max-w-md mx-auto">
                            @csrf
                            <button type="submit" class="w-full bg-rose-600 hover:bg-rose-700 text-white py-4 rounded-2xl font-bold shadow-lg shadow-rose-200 transition-all transform hover:-translate-y-1">
                ...
                @elseif($return && $return->status === 'completed')
                    <!-- STAGE 4: RETUR JUAL BELI SELESAI -->
                    <div class="text-center py-10 space-y-6">
                        <div class="w-20 h-20 bg-rose-600 text-white rounded-full flex items-center justify-center mx-auto mb-4 shadow-lg shadow-rose-200">
                            <i data-lucide="check" class="w-10 h-10"></i>
                        </div>
                        <h3 class="text-2xl font-black text-slate-900">Retur Jual Beli Selesai!</h3>
                        
                        <div class="bg-slate-50 p-6 rounded-3xl text-left border border-slate-100 max-w-md mx-auto space-y-4">
                            <p class="text-[10px] text-slate-400 uppercase font-black tracking-widest border-b pb-2">Rangkuman Resolusi</p>
                            
                            @if($return->resolution_type === 'refund')
                                <div class="space-y-2 text-xs text-slate-600">
                                    <p class="flex justify-between"><span>Resolusi:</span> <span class="font-bold text-emerald-600">Pengembalian Uang (100% Refund)</span></p>
                                    <p class="flex justify-between"><span>Total Refund:</span> <span class="font-black text-slate-900 text-sm">Rp {{ number_format($return->escrow_total) }}</span></p>
                                    <p class="flex justify-between"><span>Bank:</span> <span class="font-bold text-slate-700">{{ $return->buyer_refund_bank_name }}</span></p>
                                    <p class="flex justify-between"><span>Rekening:</span> <span class="font-bold text-slate-700">{{ $return->buyer_refund_bank_account }}</span></p>
                                    <p class="text-[10px] text-emerald-700 font-bold p-3 bg-emerald-50 rounded-xl mt-2 leading-relaxed">
                                        ✓ Dana refund sebesar Rp {{ number_format($return->escrow_total) }} telah berhasil ditransfer balik ke rekening Anda.
                                    </p>
                                </div>
                            @else
                                <div class="space-y-2 text-xs text-slate-600">
                                    <p class="flex justify-between"><span>Resolusi:</span> <span class="font-bold text-rose-600">Penggantian Produk Baru</span></p>
                                    <p class="flex justify-between"><span>Nomor Resi:</span> <span class="font-bold text-slate-700">{{ $return->resi_pengganti }}</span></p>
                                    <p class="text-[10px] text-rose-700 font-bold p-3 bg-rose-50 rounded-xl mt-2 leading-relaxed">
                                        ✓ Produk pengganti telah diterima dengan sukses dan konfirmasi penerimaan barang selesai.
                                    </p>
                                </div>
                            @endif
                        </div>
                        ...
                        <div class="mt-8 pt-6 border-t border-slate-200 text-left max-w-md mx-auto">
                            <p class="text-sm font-bold text-slate-800 mb-4">Berikan Rating & Ulasan untuk Produk Ini</p>
