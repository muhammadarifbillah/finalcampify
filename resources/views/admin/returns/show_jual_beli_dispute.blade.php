...
@section('title', 'Resolusi Sengketa Retur Jual-Beli')
...
<div class="max-w-6xl mx-auto space-y-6">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
        <div class="flex items-center gap-4">
            <a href="{{ route('admin.returns.jual_beli') }}" class="px-4 py-2 bg-gray-50 border border-gray-200 text-gray-500 text-xs font-bold rounded-lg hover:bg-gray-100 transition-all active:scale-95 flex items-center gap-2">
                <i data-lucide="arrow-left" style="width: 14px; height: 14px;"></i> Kembali
            </a>
            <div class="space-y-1">
                <div class="flex items-center gap-2">
                    <span class="px-2 py-0.5 bg-gray-100 text-gray-500 text-[10px] font-bold rounded uppercase tracking-wider">RESOLUSI #RET-{{ 88200 + $return->id }}</span>
                    <span class="px-2 py-0.5 bg-red-100 text-red-600 text-[10px] font-bold rounded uppercase tracking-wider">MARKETPLACE RETUR</span>
                </div>
        ...
        <div class="flex items-center gap-3">
            ...
            <form method="POST" action="{{ route('admin.returns.finalize', $return->id) }}">
                ...
                <button type="submit" class="px-5 py-2.5 bg-[#0f6b52] text-white text-[13px] font-bold rounded-lg flex items-center gap-2 hover:bg-[#0c5843] transition-all shadow-sm active:scale-95">
                    <i data-lucide="check-circle" style="width: 16px; height: 16px;"></i> Cairkan Dana & Tutup
                </button>
            </form>
        </div>
    </div>
        ...
        <div class="lg:col-span-7 space-y-6">
            ...
            <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
                <div class="flex items-center justify-between mb-6">
                    <h2 class="text-sm font-black text-gray-800 uppercase tracking-wider">BUKTI KONDISI BARANG</h2>
                    <div class="flex items-center gap-4 text-[10px] font-bold uppercase tracking-widest">
                        <span class="flex items-center gap-1.5 text-emerald-600"><span class="w-2 h-2 rounded-full bg-emerald-500"></span> Pengiriman</span>
                        <span class="flex items-center gap-1.5 text-red-600"><span class="w-2 h-2 rounded-full bg-red-500"></span> Pengembalian</span>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-3">
                        <div class="relative group aspect-video rounded-xl overflow-hidden bg-gray-100 border border-gray-200">
                            <img src="{{ $return->proof_sent_image ?? 'https://via.placeholder.com/400x250?text=Foto+Awal' }}" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500">
                            <div class="absolute top-3 left-3 px-2 py-1 bg-emerald-500/90 text-white text-[9px] font-black rounded uppercase tracking-widest backdrop-blur-sm">BUKTI SELLER</div>
                        </div>
                        <p class="text-[11px] text-gray-500 leading-relaxed italic">"{{ $return->owner_notes ?? 'Kondisi barang saat dikirim seller.' }}" - <span class="font-bold text-gray-700">Penjual</span></p>
                    </div>

                    <!-- Proof Returned -->
                    <div class="space-y-3">
                        <div class="relative group aspect-video rounded-xl overflow-hidden bg-gray-100 border border-gray-200">
                            <img src="{{ $return->proof_returned_image ?? 'https://via.placeholder.com/400x250?text=Foto+Retur' }}" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500">
                            <div class="absolute top-3 left-3 px-2 py-1 bg-red-500/90 text-white text-[9px] font-black rounded uppercase tracking-widest backdrop-blur-sm">BUKTI BUYER</div>
                        </div>
                        <p class="text-[11px] text-gray-500 leading-relaxed italic">"{{ $return->renter_notes ?? 'Klaim kerusakan dari pembeli.' }}" - <span class="font-bold text-gray-700">Pembeli</span></p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden flex flex-col h-[450px]">
                <div class="p-6 border-b border-gray-100 flex items-center justify-between bg-white sticky top-0 z-10">
                    <h2 class="text-sm font-black text-gray-800 uppercase tracking-wider">LOG MEDIASI RETUR</h2>
                </div>
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-8 space-y-8 sticky top-6">
                <div class="bg-gray-50/50 p-6 rounded-2xl border border-gray-100 space-y-6">
                    ...
                    <div class="bg-[#f4f9f6] p-5 rounded-xl border border-emerald-100 text-center space-y-1">
                        <div class="text-[9px] font-black text-emerald-600 uppercase tracking-widest">Total Escrow Marketplace</div>
                    ...
                    <form method="POST" action="{{ route('admin.returns.update', $return->id) }}" id="updateFeeForm" class="space-y-5">
                        ...
                        <div class="p-4 bg-white rounded-xl border border-gray-100 shadow-sm space-y-3">
                            <div class="flex items-center gap-2">
                                ...
                                <label class="text-[10px] font-black text-gray-600 uppercase tracking-wider">Potongan Refund (Damage/Service Fee)</label>
                            </div>
                            ...
                            <p class="text-[9px] text-gray-400 italic leading-relaxed">
                                Nominal ini akan dipotong dari dana pengembalian pembeli dan diberikan kepada penjual.
                            </p>
                        </div>
                        ...
                        @if($return->status !== 'completed')
                            <button type="submit" class="w-full py-3.5 bg-gray-900 text-white text-[11px] font-black uppercase tracking-widest rounded-xl hover:bg-black transition-all active:scale-[0.98] shadow-lg flex items-center justify-center gap-2">
                                <i data-lucide="refresh-cw" style="width: 14px; height: 14px;"></i> Update Perhitungan
                            </button>
                        @endif
                    </form>
                </div>
                ...
                <!-- Settlement Summary -->
                <div class="space-y-5">
                    <h2 class="text-[10px] font-black text-gray-400 uppercase tracking-widest text-center">RINGKASAN RESOLUSI</h2>
                    
                    <div class="space-y-4">
                        <div class="flex items-center justify-between">
                            <div class="flex items-center gap-2">
                                <span class="w-2 h-2 rounded-full bg-emerald-500"></span>
                                <span class="text-[13px] font-bold text-gray-700">Refund ke Pembeli</span>
                            </div>
                        ...
                        <div class="flex items-center justify-between">
                            <div class="flex items-center gap-2">
                                ...
                                <span class="text-[13px] font-bold text-gray-700">Kompensasi Penjual</span>
                            </div>
